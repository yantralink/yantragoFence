...
void main() {
  ...
  // Demo suppression (AppConfig.demoSuppressedImeis): the demo device must
  // always render the Fence Fault / Charging Status bulbs OFF (grey outline)
  // regardless of battery percentage; every other device behaves normally.

  bool isBulbOff(Widget w) =>
      w is Icon &&
      w.icon == Icons.lightbulb_outline &&
      w.size == 48 &&
      w.color == Colors.grey;

  testWidgets('demo device IMEI keeps both bulbs OFF at battery=100',
      (tester) async {
    final now = DateTime.now();
    await _pump(
      tester,
      _overrides(
        machine: _machine(imei: '866221070994202'),
        restTelemetry: Telemetry(
            imei: '866221070994202', battery: 100, timestamp: now),
      ),
    );
    expect(find.byWidgetPredicate((w) => isBulb(w, Colors.red)), findsNothing);
    expect(find.byWidgetPredicate((w) => isBulb(w, Colors.green)), findsNothing);
    expect(find.byWidgetPredicate(isBulbOff), findsNWidgets(2));
  });

  testWidgets('demo device IMEI keeps both bulbs OFF at battery=10',
      (tester) async {
    final now = DateTime.now();
    await _pump(
      tester,
      _overrides(
        machine: _machine(imei: '866221070994202'),
        restTelemetry: Telemetry(
            imei: '866221070994202', battery: 10, timestamp: now),
      ),
    );
    expect(find.byWidgetPredicate((w) => isBulb(w, Colors.red)), findsNothing);
    expect(find.byWidgetPredicate((w) => isBulb(w, Colors.green)), findsNothing);
  ...
  testWidgets('non-demo device still lights bulbs at trigger values',
      (tester) async {
    final now = DateTime.now();
    await _pump(
      tester,
      _overrides(
        machine: _machine(imei: '000000000000001'),
        restTelemetry: Telemetry(
            imei: '000000000000001', battery: 100, timestamp: now),
      ),
    );
    // Fence Fault red bulb on; Charging bulb off (grey outline).
    expect(find.byWidgetPredicate((w) => isBulb(w, Colors.red)), findsOneWidget);
    expect(find.byWidgetPredicate((w) => isBulb(w, Colors.green)), findsNothing);
    expect(find.byWidgetPredicate(isBulbOff), findsOneWidget);
  });
}
