...
export default function SuperAdminCustomersPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const params: any = { name, phone, email, address, organizationId: selectedOrgId };
  ...
  const handleEdit = (c: Customer) => {
    ...
    setSelectedOrgId(c.organizationId);
  ...
  const filteredMachines = machines?.filter(m => {
    ...
    // Only show machines in the selected organization
    if (selectedOrgId && m.organizationId !== selectedOrgId) return false;
        ...
        <Card title={editingId ? 'Edit Customer' : 'Create Customer'}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">Optional. Only IN_STOCK machines in the selected organization are shown.</p>
            </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {customers.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-500">{orgName(c.organizationId)}</td>
